use crate::{
    db::{Database, DbError},
    models::{channel::Channel, message::Message, server::Server, user::{User, UserState}, MessageType},
};
...
pub async fn handle_connect_server<D: Database>(
    ...
    if !server.user_ids.contains(&user.id) {
        if let Err(e) = db.add_server_user(&server.id, &user.id, 1).await {
            ...
            socket.emit("error", &json!({"message": "Failed to add user to server"})).ok();
            return Ok(());
        }
    }
...
pub async fn handle_connect_user<D: Database>(
    ...
    if let Err(e) = db.update_user(&user).await {
        ...
        socket.emit("error", &json!({"message": "Failed to update user state"})).ok();
        return Ok(());
    }
...
pub async fn handle_chat_message<D: Database>(
    ...
    let sender = match db.get_user(&data.message.sender).await {
        ...
        Err(e) => {
            ...
            socket.emit("error", &json!({"message": "User not found"})).ok();
            return Ok(());
        }
    };
    ...
    if sender.current_channel_id.is_none() {
        ...
        socket.emit("error", &json!({"message": "You are not in any channel"})).ok();
        return Ok(());
    }
    ...
    // 廣播訊息給頻道內的所有使用者
    socket.to(format!("server_{}_channel_{}", server.id, channel_id))
        .emit("message", &json!(message))
        .await.map_err(|e| e.to_string())?;

    info!("Message sent in server({}) channel({})", server.id, channel_id);
    Ok(())
}
...
pub async fn handle_add_channel<D: Database>(
    ...
    info!("Added new channel({}) to server({})", data.channel.id, server.id);
    Ok(())
}
...
pub async fn handle_delete_channel<D: Database>(
    ...
    // 1. 先更新所有在此頻道的用戶
    let users = db.get_channel_users(&data.channel_id).await.map_err(|e| e.to_string())?;
...
pub async fn handle_join_channel<D: Database>(
    ...
    let mut channel = match db.get_channel(&data.channel_id).await {
        ...
        Err(e) => {
            ...
            socket.emit("error", &json!({"message": format!("Channel({}) not found", data.channel_id)})).ok();
            return Ok(());
        }
    };
    ...
    if let Some(current_channel_id) = &user.current_channel_id {
        socket.leave(format!("server_{}_channel_{}", server.id, current_channel_id));
    }
    ...
    if let Err(e) = db.update_channel(&channel).await {
        error!("Failed to update channel: {}", e);
        socket.emit("error", &json!({"message": "Failed to update channel"})).ok();
        return Ok(());
    }
    ...
    // 發送更新後的資料
    let channels = db.get_server_channels(&server.id).await.map_err(|e| e.to_string())?;
    let users = db.get_server_users(&server.id).await.map_err(|e| e.to_string())?;
    let messages = db.get_channel_messages(&channel.id).await.map_err(|e| e.to_string())?;


    socket.to(format!("server_{}", server.id)).emit("channels", &json!(channels)).await.map_err(|e| e.to_string())?;
    socket.to(format!("server_{}", server.id)).emit("users", &json!(users)).await.map_err(|e| e.to_string())?;
    socket.emit("messages", &json!(messages)).ok();
    socket.emit("user", &json!(user)).ok();

    info!("User({}) joined channel({}) in server({})", user.id, channel.id, server.id);
    Ok(())
}
...
async fn get_server_messages<D: Database>(db: &D, channel_id: &str) -> Result<Vec<Message>, DbError> {
